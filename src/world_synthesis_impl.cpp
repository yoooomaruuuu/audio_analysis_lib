...
world_synthesis_impl::world_synthesis_impl(int init_fs, float init_frame_period)
	: param(new world_parameters()), harvest(new harvest_f0(init_fs, init_frame_period)),
	  cheaptrick(new cheaptrick_sp(init_fs)), d4c(new d4c_ap(init_fs)), frame_length((init_frame_period / 1000.0) * init_fs),
      synthesizer({0})
{
	param->fs = init_fs;
	param->frame_period = init_frame_period;
	param->f0_length = harvest->get_f0_sample_num(frame_length);
...
void world_synthesis_impl::realtime_synth(double f0_shift_size, double* frame_data, double* output_data)
{
	...
	Synthesis2(&synthesizer);
}


void world_synthesis_impl::f0_estimate(double* frame_data)
{
	harvest->f0_estimate(frame_data, frame_length, param->time_axis, param->f0);
}
void world_synthesis_impl::sp_estimate(double* frame_data)
{
	cheaptrick->spectrogram_estimated(frame_data, frame_length, param->time_axis, param->f0, param->f0_length, param->spectrogram);
}
void world_synthesis_impl::ap_estimate(double* frame_data)
{
	d4c->aperiodicity_estimated(frame_data, frame_length, param->time_axis, param->f0, param->f0_length, param->fft_size, param->aperiodicity);
}
